...
class Connection {
    ...
    if(supabase){
      this.channel = supabase
        ...
        .on('postgres_changes',{event:'INSERT',schema:'public',table:'realtime_events'},payload=>{
          const row = payload.new as {entity_type:string;entity_id:string;payload:any};
          const key = `${row.entity_type}:${row.entity_id}`;
          if(!this.subscriptions.has(key)) return;
          const message = {v:1,type:'entity.update',payload:{entity_type:row.entity_type,entity_id:row.entity_id,data:row.payload}};
  ...
  disconnect(){ if(this.channel&&supabase) supabase.removeChannel(this.channel); this.listeners.clear(); connections.delete(this.connectionId); }
}
...
async function request(method:string,url:string,data?:any):Promise<ApiResponse>{
  ...
  if(method==='GET' && data){
    const qs=new URLSearchParams(); Object.entries(data).forEach(([k,v])=>v!=null&&qs.set(k,String(v)));
    target += `${url.includes('?')?'&':'?'}${qs.toString()}`;
  }else if(data!==undefined){ options.body=JSON.stringify(data); }
  const res=await fetch(target,options);
  const body=await res.json().catch(()=>({error:`Request failed (${res.status})`}));
  if(!res.ok){ const err=new Error(body?.error||body?.message||`Request failed (${res.status})`) as Error & {status?:number;data?:any}; err.status=res.status;err.data=body;throw err; }
  return {data:body};
}
