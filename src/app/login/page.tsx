    ...
    setTimeout(() => {
      ...
      if (result.success) {
        ...
        if (saved) {
          ...
          router.push(ROLE_HOME[user.role as keyof typeof ROLE_HOME] || "/")
        }
      } else {
  ...
  return (
    <div className="min-h-[100dvh] bg-white flex flex-col pt-16 px-6 font-sans">
      ...
      >
        ...
        <h2 className="text-3xl font-bold text-[#1a2b4b] tracking-tight mb-2">Welcome back</h2>
      ...
      >
        {/* Social Buttons */}
        <motion.button variants={item} className="w-full flex items-center justify-center gap-3 py-3.5 px-4 bg-white border border-gray-200 rounded-xl shadow-sm hover:bg-gray-50 transition-colors font-semibold text-[15px] text-[#1a2b4b]">
        ...
        <motion.form variants={item} onSubmit={handleSignIn} className="flex flex-col gap-5">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              ...
              <Link href="#" className="text-[13px] font-semibold text-[var(--color-brand-primary)] hover:underline">Forgot password?</Link>
            </div>
            <input
              type="password"
              value={password}
          ...
          <motion.button
            ...
            type="submit"
            disabled={loading}
            className="w-full mt-2 bg-[var(--color-brand-primary)] text-white font-bold text-[15px] py-4 rounded-xl shadow-[0_4px_14px_0_rgba(195,59,34,0.39)] hover:bg-[var(--color-brand-primary)]/90 transition-all flex items-center justify-center gap-2 disabled:opacity-70"
          >
            {loading ? (
        ...
        <motion.div variants={item} className="mt-4 text-center">
          <p className="text-[15px] font-medium text-gray-500">
            ...
            <Link href="/signup" className="text-[var(--color-brand-primary)] font-semibold hover:underline">Sign up</Link>
          </p>
        </motion.div>
      </motion.div>
    </div>
