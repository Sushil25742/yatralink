...
const roleLabel: Record<Role, string> = {
  ...
  engineer: "Route Mapping Engineer",
};
...
export default function AccessPortal() {
  ...
  const completeAuth = (data: { session_id: string; user: UserSession }) => {
  ...
  const logout = async () => {
    if (sessionId)
      await api
        .post("/api/demo-auth/logout", { session_id: sessionId })
        .catch(() => {});
...
function Welcome({
  ...
  const transition = reduceMotion
    ...
    : {
        duration: 0.48,
        ease: [0.22, 0.8, 0.22, 1] as [number, number, number, number],
      };
  return (
    <main className="onboarding-shell">
      <AnimatePresence mode="wait" initial={false}>
        {step === 0 ? (
          <motion.section
            key="splash"
            className="onboarding-screen onboarding-splash"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            transition={transition}
          >
            <div className="onboarding-image onboarding-image--mountain" />
            <div className="onboarding-shade" />
            <motion.div
              className="onboarding-splash__brand"
            ...
            <motion.div
              className="onboarding-splash__footer"
              initial={reduceMotion ? false : { opacity: 0, y: 12 }}
              animate={{ opacity: 1, y: 0 }}
            >
              <p>Authentic Nepal, Locally Lived</p>
          ...
          <motion.section
            key="discover"
            className="onboarding-screen onboarding-story"
          ...
          >
            <header className="onboarding-topbar">
              <strong>YatraLink</strong>
              <button onClick={onLogin}>Skip</button>
            </header>
            <div className="onboarding-story__image onboarding-story__image--culture" />
            <div className="onboarding-story__content">
              ...
              <p>
                Find living heritage, local makers and cultural experiences
          ...
          <motion.section
            key="confidence"
            className="onboarding-screen onboarding-confidence"
            initial={reduceMotion ? false : { opacity: 0, x: 34 }}
            animate={{ opacity: 1, x: 0 }}
            transition={transition}
          >
            <div className="onboarding-confidence__image" />
            <header className="onboarding-confidence__top">
              <strong>YatraLink</strong>
              <button onClick={onLogin}>Skip</button>
            </header>
            <div className="onboarding-confidence__sheet">
              <div className="onboarding-dots onboarding-dots--center">
                <i />
                <i />
                <i className="active" />
              </div>
              ...
              <p>
                Crowd-aware timing, trusted local experiences and an AI-planned
                timeline built around your trip.
              </p>
              <button className="onboarding-primary" onClick={onLogin}>
                Get Started <ArrowRight />
              </button>
              <button className="onboarding-signup" onClick={onSignup}>
                New to YatraLink? <b>Create traveler account</b>
              </button>
            </div>
          </motion.section>
        )}
...
function Login({
  ...
  onBack: () => void;
  onSuccess: (d: { session_id: string; user: UserSession }) => void;
  onSignup: () => void;
}) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const { data } = await api.post("/api/demo-auth/login", {
        email,
        password,
      });
      onSuccess(data);
  ...
  return (
    <AuthLayout
      title="Welcome back"
      subtitle="Sign in to your YatraLink workspace."
      onBack={onBack}
    >
      ...
      <div className="auth-switch">
        New to YatraLink? <button onClick={onSignup}>Create an account</button>
      </div>
      <div className="demo-role-note">
        ...
        <div>
          ...
          <p>
            Your account automatically opens the correct Traveler, Operator,
            Superadmin or Route Engineer workspace.
          </p>
        </div>
      </div>
    </AuthLayout>
...
function Signup({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const { data } = await api.post("/api/demo-auth/signup", {
        name,
        email,
        password,
      });
  ...
  return (
    <AuthLayout
      ...
      subtitle="New accounts start with Traveler access."
      onBack={onBack}
    >
      <form className="access-form" onSubmit={submit}>
        <label>
      ...
      <div className="auth-switch">
        Already registered? <button onClick={onLogin}>Sign in</button>
      </div>
    </AuthLayout>
  );
}
    ...
    <main className="auth-page">
      <section className="auth-visual">
        ...
        <div>
          ...
          <p>
            Smarter timing creates better journeys and stronger local economies.
          </p>
        </div>
      </section>
...
function SettingsPage({
  ...
  const [tab, setTab] = useState<
    "general" | "privacy" | "notifications" | "language"
  >("general");
  ...
  return (
    <main
      className={`settings-page ${settings.dark_mode ? "settings-page--dark" : ""}`}
    >
      ...
      <div className="settings-layout">
        <aside>
          <div className="settings-user">
            <span>{settings.name.slice(0, 1).toUpperCase()}</span>
        ...
        <section className="settings-content">
          <h1>
            {tab === "general"
              ? "Make YatraLink work your way."
              : tab === "privacy"
                ? "Control what you share."
                : tab === "notifications"
                  ? "Choose the alerts that matter."
                  : "Choose your preferred language."}
          </h1>
          <p>These preferences persist for this account across sessions.</p>
          {error && (
            <div className="access-error">
              <AlertTriangle />
              {error}
            ...
            <div className="settings-grid">
                ...
                <>
                  ...
                  <div className="settings-toggle">
                    <div>
                      ...
                      <p>Use a darker account settings workspace.</p>
                    </div>
                    <Switch
                      value={settings.dark_mode}
                      onChange={(v) =>
                        setSettings({ ...settings, dark_mode: v })
                      }
                    />
                  </div>
                </>
              ...
              {tab === "privacy" && (
                <div className="settings-toggle">
                  <div>
                    ...
                    <p>
                      Opt in to privacy-preserving location features such as
                      Quiet Nearby.
                    </p>
                  </div>
                  <Switch
                    ...
                    onChange={(v) =>
                      setSettings({ ...settings, location_sharing: v })
                    }
                  />
                </div>
              )}
              {tab === "notifications" && (
                <div className="settings-toggle">
                  <div>
                    ...
                    <p>
                      Surface crowd changes that affect your planned
                      destinations.
                    </p>
                  </div>
                  <Switch
                    ...
                    onChange={(v) =>
                      setSettings({ ...settings, crowd_alerts: v })
                    }
                  />
                </div>
              )}
              {tab === "language" && (
                <label>
                  ...
                  <small>
                    Preference is persisted; full interface translation remains
                    a prototype extension.
                  </small>
                </label>
              )}
              <button
                className="access-primary settings-save"
                disabled={busy}
                onClick={save}
              >
                {saved ? (
...
function EngineerPanel({
  ...
  useEffect(() => {
    api
      .get("/api/engineer-map", { session_id: sessionId })
      .then(({ data }) => {
  ...
  const connect = () => {
    ...
    const a = nodes.find((n) => n.id === selected[0]),
      b = nodes.find((n) => n.id === selected[1]);
    if (!a || !b) return;
    setRoutes((x) => [
      ...x,
      {
